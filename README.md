...
Dự án kiểm thử tự động cho ứng dụng **Plant Identifier** sử dụng framework Appium và Pytest. Hệ thống tập trung kiểm tra độ ổn định và chính xác của tính năng nhận dạng AI (thực vật, nấm, côn trùng, cá) 
...
appium
```
*(Đảm bảo điện thoại đã được kết nối với máy tính, bật chế độ Developer Options > USB Debugging và màn hình đang mở khóa).*
...
```bash
pytest src/tests/test_plant_scan.py
```
